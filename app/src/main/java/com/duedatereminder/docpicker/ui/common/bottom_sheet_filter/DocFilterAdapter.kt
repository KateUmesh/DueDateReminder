...
class DocFilterAdapter: RecyclerView.Adapter<DocFilterAdapter.DocVewHolder>(), AnkoLogger {
    ...
    inner class DocVewHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        var item_doc_filter_ivc:ImageViewCheckable = itemView.findViewById(R.id.item_doc_filter_ivc)
        var item_doc_filter_doc_type:TextView = itemView.findViewById(R.id.item_doc_filter_doc_type)
        var item_doc_filter_file_ext:TextView = itemView.findViewById(R.id.item_doc_filter_file_ext)
        fun bind(pData: DocFilterModel){

            setDrawableForMime(itemView, pData.docType)
            item_doc_filter_ivc.isChecked = pData.isSelected
            item_doc_filter_doc_type.text = pData.docType
        ...
        private fun setDrawableForMime(itemView: View, docType: String) {
            ...
            when(docType){
                DocPicker.DocTypes.IMAGE->{
                    ...
                    item_doc_filter_file_ext.text = DocConstants.docTypeMapLabel()[docType]
                }
            ...
            item_doc_filter_file_ext.background = mDrawable
        }
    }
